...
public class ReplyService {
	...
	@Transactional
	// -> Transactional: DB에서 검색한 엔터티를 수정하면 트렌젝션이 끝나는 시점에 update 쿼리가 자동으로 실행됨.
	public void update(long id, ReplyUpdateDto dto) {
	    log.info("update(id={}, dto={})", id, dto);
	    
	    // 1. 댓글 아이디로 DB에서 엔터티를 검색(select):
	    Reply entity = replyRepository.findById(id).orElseThrow(); //findById는 단일id 하나만 리턴하기 때문에 옵셔널로 orElseThorw를 넣어주어야 한다. 
	    // 검색한 아이디가 있으면 entity변수에 저장, 없으면 orElseThrow를 통해 익셉션을 발생시킴
	...
	@Transactional(readOnly = true) // readOnly는 셀렉트한 엔터티가 수정되지않는 것 일때만 사용.
